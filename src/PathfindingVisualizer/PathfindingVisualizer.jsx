...
import {showPopUp} from "./piyushpop/piyushpop.jsx";
...
let FINISH_NODE_COL = 31;
 ...
 class PathfindingVisualizer extends Component {
  ...
  visualizeDijkstra() {
    this.setState({ topMessage: "Creator : Piyush" });
  ...
  // *****************this is go from the endnode to the start nodeand draw the yellow line**********************
  ...
  render() {
    ...
    } else if (topMessage === " Creator : piyush ") {
    ...
    if (topMessage === "Creator : Piyush") {
      textBox = null;
    } else if (topMessage === "Shortest Path") {
      textBox = (
        <div
          className="buttonContainer"
          style={{ width: "30%", margin: "0 auto" ,color : "white" }}
        >
          {button_task}
        </div>
      );
...
const createNode = (col, row) => {
  // return all the values of one object in one node by making class component
  return {
    col,
    row,
...
// ********************this code is use updating the grid by using toggeld  ******************************************************
const getNewGridWithWallToggled = (grid, row, col) => {
  const newGrid = [...grid]; // copy of all grid
  const node = newGrid[row][col]; //copy all states of old grid
  const newNode = {
    ...node, // copying other properties of the node
    isWall: !node.isWall,
  };
  newGrid[row][col] = newNode;//update the new grid by use toggled
  return newGrid;
};
...
// ************************use for toggeld the weight and wall and return new grid***********************************
